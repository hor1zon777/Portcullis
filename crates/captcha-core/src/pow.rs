...
// ────────────────────── 双阶段 PoW 算法 ──────────────────────
//
// Phase 1（一次性）：Argon2id(challenge.id, salt) → base_hash
//   保留内存硬化特性，GPU 每个 challenge 必须付出 4 MiB 内存成本。
//
