...
| 4 | v1.4.0 | CaptchaToken 客户端身份绑定（opt-in） | ✅ 已完成 |
| 5 | v1.5.0 | 服务端密钥与审计硬化 | ✅ 已完成 |
| **6** | **v1.6.0** | **供应链与分发签名** | 🔜 下一个 |
...
## v1.4.0 — CaptchaToken 客户端身份绑定（✅ 已完成）
...
| 4 | **反代透传文档** | `docs/DEPLOY.md`：启用 `bind_token_to_ip` 时必须正确转发 `X-Real-IP` / `X-Forwarded-For`，否则流量全挂 | ✅（含 Nginx / Caddy / Apache / Traefik 示例） |
| 5 | **管理面板开关 UI** | `admin-ui/src/pages/Sites.tsx` 加两个 checkbox + 表格绑定列 | ✅ |
| 6 | **集成测试 & 单元测试** | 21 个新测试（9 integration + 12 token unit） | ✅ |

### 工作量估计：2-3 天 — 实际约 1 天完成
...
## v1.5.0 — 服务端密钥与审计硬化（✅ 已完成）

### 范围

| # | 项目 | 破坏性 | 状态 |
|---|---|---|---|
| 1 | **DB `secret_key` 改 hash 存储** | ⚠️ schema migration | ✅ 启动迁移 + 一次性明文返回 |
| 2 | **admin 操作审计表** | 新增表 | ✅ `admin_audit` + 管理面板审计页 + 500 条分页 |
| 3 | **admin 登录限流 + IP ban** | 无破坏 | ✅ 30 次失败触发 15 分钟 ban（429 响应） |
| 4 | **`CAPTCHA_SECRET` 双 key 轮换** | 无破坏 | ✅ `secret_previous` + `verify_sig_any` + `site_secret::verify_any` |
| 5 | **Admin 操作 webhook** | 可选 | ✅ `reqwest` + rustls-tls + 2s/3s 超时 |

### 工作量估计：3-5 天 — 实际约 1 天完成

### 迁移说明
`secret_key` hash 化是向后不兼容的 schema 变更。启动时检测到明文行自动一次性 hash，**这要求管理员在 v1.4.x 之前已经保留了 secret_key 的原文备份**（因为 hash 化后 DB 里拿不回原文）。升级流程详见 [UPGRADING.md](UPGRADING.md) v1.4 → v1.5 章节。
